...
struct Control {
   DWORD dwType;               //0x00
   DWORD *_1;                  //0x04 // unsure? definitely a ptr but not obvious, usually points to 6 when dwType is 4 I think
   ...
   void (__fastcall *func)(Control*); //0x1C
   DWORD _3;                  //0x20
...
/*struct Control {
	...
	void (__fastcall *func)(Control*); // +1c
	BYTE __1[3];					//0x1D
	DWORD _2;						//0x20
	DWORD fnClick;					//0x24
	DWORD _3[5];					//0x38
...
// Thanks to 99Elite for posting informations about this list!
...
typedef list<ItemDesc*> ItemDescList;
typedef map<String, int> njipMap;
