...
INT CMap::GetUnitCellNumber(DWORD dwClassId, DWORD dwLevelNo)
{
    ...
    if(dwClassId == 402 && dwLevelNo == MAP_A2_CANYON_OF_THE_MAGI)
        return 0;
    if(dwClassId == 267 && dwLevelNo != MAP_A3_KURAST_DOCKS && dwLevelNo != MAP_A4_THE_PANDEMONIUM_FORTRESS)
        return 0;
    if(dwClassId == 376 && dwLevelNo == MAP_A4_RIVER_OF_FLAME)
        return 376;
