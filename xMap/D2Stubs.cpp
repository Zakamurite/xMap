...
LONG WINAPI GameWindowEvent(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (GameReady ())
	{
		...
		{
			if (wParam == VK_LEFT) ScrollMap(-2, 0);
			if (wParam == VK_RIGHT) ScrollMap(+2, 0);
			if (wParam == VK_UP) ScrollMap(0, -2);
			if (wParam == VK_DOWN) ScrollMap(0, +2);
...
BOOL __fastcall InfravisionPatch3(UnitAny *unit)
{
	...
	switch(unit->dwType) {
	case UNIT_PLAYER:
		if (!unit->dwMode || unit->dwMode == 0x11 || bInfravision) return FALSE;
		break;
	case UNIT_MONSTER:
	...
	if (curlvl && !D2COMMON_GetFunUnk_5(curlvl->dwLevelNo)) return FALSE;
...
BYTE __fastcall MonsterBlobColPatch2(UnitAny *pUnit)
{
	if (bShowMonsters)
	{
		...
		{
			if (pUnit->pMonsterData->fBoss & 1) nColor = nBossMonsterColor;
			else if(pUnit->pMonsterData->fChamp & 1) nColor = nChampionMonsterColor;
			else if(pUnit->pMonsterData->fMinion & 1) nColor = nMinionMonsterColor;
			else nColor = nNormalMonsterColor;
		}
...
{
	if (!fPlayerInTown && ViewTestUnit(unit)) {//uviewing
...
{
	if (!bInputLinePatch) return FALSE;
	if(!pD2WinEditBox) {
